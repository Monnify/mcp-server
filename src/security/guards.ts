...
export function assertEnvironmentConsistency(env: Env): void {
...
function assertApiKeyMatchesEnvironment(apiKey: string, isProductionEnv: boolean): void {
...
export function isOperationAllowed(
  ...
  if (!env.MONNIFY_ALLOWED_OPERATIONS) return true;
  return env.MONNIFY_ALLOWED_OPERATIONS.split(",")
    .map((s) => s.trim() as OperationCategory)
    .includes(category);
}
