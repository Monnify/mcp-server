import { z } from "zod";
import { CreateMandateBodySchema, DebitMandateBodySchema, } from "../compat.js";
export const CreateMandateInputSchema = CreateMandateBodySchema.extend({
    mandateReference: z
        .string()
        .min(1)
        .max(64)
        .describe("Your unique reference for this mandate. Used for idempotency — resubmitting the same reference will not create a duplicate mandate."),
    mandateAmount: z
        .number()
        .positive()
        .describe("Total lifetime amount debitable on this mandate in Naira (NGN)."),
    ...
    customerAccountBankCode: z
        .string()
        .length(3)
        .describe("3-digit bank code of the customer's bank. Call monnify_get_supported_banks to retrieve."),
    customerName: z
        .string()
        .min(1)
        .describe("Full name of the customer — must match the bank account holder name."),
    customerEmailAddress: z
        .string()
        .email()
        .describe("Customer email address — Monnify will send mandate notifications here."),
    customerPhoneNumber: z
        .string()
        .describe("Customer phone number in international format e.g. 2348012345678."),
    customerAddress: z
        .string()
        .min(1)
...
export const DebitMandateInputSchema = DebitMandateBodySchema.extend({
    paymentReference: z
        ...
        .min(1)
        .max(64)
        .describe("Your unique idempotency reference for this debit. The same reference can be safely resubmitted to check status without double-charging."),
    mandateCode: z
        .string()
        .min(1)
        .describe("Monnify's generated mandate identifier (e.g. 'MTDD|01HY8WMN8JYKDRJC67QPQVS1N0'). Returned in the mandateCode field from monnify_get_mandate_status. The mandate status MUST be ACTIVATED."),
    debitAmount: z
        .number()
        .positive()
...
export const CancelMandateInputSchema = z.object({
    mandateCode: z
        .string()
        .min(1)
        .describe("Monnify's generated mandate identifier (e.g. 'MTDD|01HY8WMN8JYKDRJC67QPQVS1N0'). Returned in the mandateCode field from monnify_get_mandate_status. This is irreversible — the customer will need a new mandate to authorise future debits."),
});
