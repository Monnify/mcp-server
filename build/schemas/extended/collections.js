import { z } from "zod";
import { InitiatePaymentBodySchema, ReserveAccountBodySchema, CreateInvoiceBodySchema, ProcessRefundBodySchema, ChargeCardBodySchema, AuthoriseCardOtpBodySchema, AuthoriseCard3dsBodySchema, } from "../compat.js";
export const InitiatePaymentInputSchema = InitiatePaymentBodySchema.extend({
    ...
    paymentReference: z
        ...
        .min(1)
        .max(64)
        .describe("Your unique idempotency reference for this payment. Reusing the same reference safely deduplicates retries."),
    paymentDescription: z
        .string()
        .max(100)
        .describe("Human-readable description shown to the customer on the payment page."),
    currencyCode: z
        .enum(["NGN"])
        .default("NGN")
    ...
    contractCode: z
        .string()
        .min(1)
        .describe("Your Monnify contract code. Found in Dashboard → Settings → Contract Codes."),
    redirectUrl: z
        .string()
        .url()
        .optional()
    ...
    incomeSplitConfig: z
        ...
        .optional()
        .describe("Income split configuration — distributes the payment across multiple sub-accounts at settlement. Omit to credit the full amount to the primary account."),
});
export const ReserveAccountInputSchema = ReserveAccountBodySchema.extend({
    accountReference: z
        ...
        .min(1)
        .max(64)
        .describe("Your unique reference for this reserved account. Used for idempotency."),
    accountName: z
        .string()
        .min(1)
    ...
    bvn: z
        ...
        .describe("Customer's 11-digit Bank Verification Number (BVN) issued by the Central Bank of Nigeria. Required for KYC compliance."),
    getAllAvailableBanks: z
        .boolean()
        .describe("If true, Monnify returns virtual account numbers across all available banking partners. Set to false to restrict to preferredBanks only."),
    preferredBanks: z
        .array(z.string())
        .describe("Array of bank codes for the preferred virtual account banks (e.g. ['50515'] for Moniepoint MFB). Required even when getAllAvailableBanks is true — used as the fallback/priority set."),
    nin: z
        .string()
        .regex(/^\d{11}$/, "NIN must be exactly 11 digits")
        .optional()
        .describe("Customer's 11-digit National Identification Number (NIN) issued by NIMC. Optional but recommended for enhanced KYC."),
    restrictPaymentSource: z
        .boolean()
        .optional()
        .default(true)
        .describe("If true (default), only BVNs/accounts listed in allowedPaymentSources can fund this account. Set to false to allow payments from any source."),
    allowedPaymentSources: z
        ...
        .optional()
        .describe("Whitelist of payers allowed to fund this reserved account. Only enforced when restrictPaymentSource is true."),
    incomeSplitConfig: z
        .array(z.object({
        subAccountCode: z
            .string()
        ...
        .optional()
        .describe("Income split configuration — distributes received payments across multiple sub-accounts. Omit to credit the full amount to the primary account."),
});
export const GetTransactionStatusInputSchema = z.object({
    ...
    transactionReference: z
        .string()
        .optional()
        .describe("Monnify's transaction reference. Provide either this or paymentReference."),
});
export const CreateInvoiceInputSchema = CreateInvoiceBodySchema.extend({
    ...
    expiryDate: z
        .string()
        .describe("Invoice expiry date in ISO 8601 format (YYYY-MM-DDTHH:mm:ss). After this date the invoice can no longer be paid."),
    paymentMethods: z
        .array(z.enum(["CARD", "ACCOUNT_TRANSFER", "USSD", "PHONE_NUMBER"]))
        .optional()
        .describe("Allowed payment methods. Omit to allow all available methods."),
    ...
    accountReference: z
        .string()
        .optional()
        .describe("Your unique reference for the reserved account linked to this invoice. Required when creating a Static Invoice (one that reuses a reserved account)."),
});
export const ChargeCardInputSchema = ChargeCardBodySchema.extend({
    transactionReference: z
        .string()
        .min(1)
        .describe("Monnify transaction reference from monnify_initiate_payment. Links this card charge to the pending transaction."),
    collectionChannel: z
        .string()
        .default("API_NOTIFICATION")
        .describe("Collection channel. Defaults to API_NOTIFICATION for direct card charges."),
    card: z.object({
        number: z
            .string()
        ...
        expiryMonth: z
            ...
            .describe("Card expiry month in 2-digit format (e.g. '09' for September)."),
        expiryYear: z
            .string()
        ...
        pin: z
            ...
            .describe("4-digit card PIN. Required for Nigerian-issued cards. Transmit only over TLS — never log."),
        cvv: z
            .string()
            .regex(/^\d{3}$/, "CVV must be 3 digits")
            .describe("3-digit card CVV on the back of the card. Never log or store."),
    }),
    deviceInformation: z.object({
        ...
        httpBrowserTimeDifference: z
            .string()
            .describe("UTC offset of the browser's timezone (e.g. '-60'). Pass empty string if unknown."),
...
export const AuthoriseCardOtpInputSchema = AuthoriseCardOtpBodySchema.extend({
    transactionReference: z
        .string()
        .min(1)
        .describe("Monnify transaction reference returned by monnify_charge_card. Must match the transaction awaiting OTP."),
    tokenId: z
        .string()
        .min(1)
        .describe("Token ID returned in the otpData from monnify_charge_card when status is OTP_AUTHORIZATION_REQUIRED."),
    token: z
        .string()
        .min(4)
        .max(8)
        .describe("OTP sent to the customer's registered phone number by their bank. Collect from the customer and pass here."),
});
export const AuthoriseCard3dsInputSchema = AuthoriseCard3dsBodySchema.extend({
    transactionReference: z
        .string()
        .min(1)
        .describe("Monnify transaction reference returned by monnify_charge_card when status is THREE_DS_TRANSACTION_INITIATED."),
    apiKey: z
        .string()
        .min(1)
        .describe("The merchant's monnify provided API key. Required to complete 3DS authorisation."),
    card: z
        .object({
        ...
        .optional()
        .describe("Card details — only required if not already captured in the initial charge."),
});
export const PayWithBankTransferInputSchema = z.object({
    transactionReference: z
        .string()
        .min(1)
        .describe("Monnify transaction reference returned by monnify_initiate_payment. Required to link this bank transfer to an existing pending transaction."),
    bankCode: z
        .string()
        .length(3)
        .optional()
        .describe("3-digit bank code for USSD string generation (e.g. '058' for GTBank). Optional — omit to let Monnify select the bank. Call monnify_get_supported_banks to retrieve the full list."),
});
export const ChargeCardTokenInputSchema = z.object({
    cardToken: z
        .string()
        .min(1)
        .describe("Card token retrieved from the Get Transaction Status response after a previous successful card charge. Use monnify_get_transaction_status to obtain it."),
    amount: z
        .number()
        .positive()
    ...
    customerEmail: z
        .string()
        .email()
        .describe("Email address of the customer — must match the email used in the original card charge that produced this token."),
    paymentReference: z
        .string()
        .min(1)
        .max(64)
...
export const ProcessRefundInputSchema = ProcessRefundBodySchema.extend({
    transactionReference: z
        .string()
        .min(1)
        .describe("The Monnify transaction reference of the original payment to refund."),
    refundReference: z
        .string()
        .min(1)
        .max(64)
        .describe("Your unique idempotency reference for this refund. Reusing the same reference is safe and will not create a duplicate refund."),
    refundAmount: z
        .number()
        .positive()
        .describe("Amount to refund in Naira. Cannot exceed the original payment amount."),
    refundReason: z
        .enum([
        "CUSTOMER_REQUEST",
        "ORDER_CANCELLATION",
    ...
    destinationAccountBankCode: z
        ...
        .length(3)
        .optional()
        .describe("3-digit bank code for the destination account. Required when destinationAccountNumber is provided. Call monnify_get_supported_banks to retrieve."),
});
