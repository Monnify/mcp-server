...
KEY OUTPUT FIELDS: mandateReference, mandateCode, mandateStatus (PENDING_AUTHORIZATION | ACTIVATED | CANCELLED | REJECTED), mandateAmount, mandateType, customerName, customerAccountNumber, startDate, endDate, authorizationLink.`,
  inputSchema: zodToJsonSchema(GetMandateStatusInputSchema) as Tool["inputSchema"],
};
...
async function handler(args: unknown): Promise<McpToolResult> {
  try {
    ...
    const result = await apiGet<Record<string, unknown>>(
      ...
      { mandateReferences: parsed.mandateReferences }
    );
    const sanitised = sanitiseMandateStatusResponse(result);
    return {
      content: [{ type: "text", text: getResponseFormat() === "json" ? JSON.stringify(sanitised, null, 2) : formatMandateStatus(sanitised as Record<string, unknown>) }],
    };
  } catch (error) {
