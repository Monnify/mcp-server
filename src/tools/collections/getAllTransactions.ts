...
const definition: Tool = {
  name: "monnify_get_all_transactions",
  description: `Returns a paginated list of all transactions on your integration, with optional filters.

WHEN TO USE: For reporting, reconciliation, or finding transactions when you have partial information (e.g. date range, customer email, amount range, or status) rather than a specific reference.
...
KEY OUTPUT FIELDS: content (array of transactions — each with transactionReference, paymentReference, amountPaid, totalPayable, paymentStatus, paidOn, paymentMethod, currencyCode), totalElements, totalPages, size, number (current page).`,
  inputSchema: zodToJsonSchema(InputSchema) as Tool["inputSchema"],
};

async function handler(args: z.infer<typeof InputSchema>): Promise<McpToolResult> {
  try {
    ...
    const safeContent = (content as Array<Record<string, unknown>>).map((tx) => ({
    ...
    return errorResult(`monnify_get_all_transactions failed: ${String(error)}`);
  }
}
